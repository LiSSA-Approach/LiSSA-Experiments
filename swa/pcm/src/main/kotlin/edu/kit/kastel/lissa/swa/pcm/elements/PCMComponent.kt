...
data class PCMComponent(
    ...
    companion object {
        ...
        private fun handleRoles(
            ...
            val roleEntities =
                component
                    .listProperties(ontology.getProperty(roleEntityId).get())
                    .mapWith { s -> ontology.getIndividualByIri(s.`object`.asResource().uri).get() }
                    .toList()

            val interfaceIds =
                roleEntities
                    .map { r ->
                        r.getProperty(ontology.getProperty(interfaceProperty).get())
                            .`object`
                            .asResource()
                            .uri
                    }
                    .map { iri -> ontology.getIndividualByIri(iri).get().pcmId(ontology) }
